...
_mainScript_() {
  ...
  if command -v zsh &>/dev/null; then
    ...
    if [[ "$OSTYPE" =~ "linux-gnu"* ]]; then
      ...
      _execute_ "sudo apt-get upgrade"
      _execute_ "sudo apt-get install zsh"
    elif [[ "$OSTYPE" == "darwin"* ]]; then
      _execute_ "brew install zsh"
    else
      warning "Could not install 'zsh'"
    fi
  fi
  ...
  if command -v ${binroot}/zsh >/dev/null; then
    if [[ $SHELL != ${binroot}/zsh ]]; then
      info "Making ${binroot}/zsh your default shell"
      if ! grep -q "${binroot}/zsh" </etc/shells; then
        _execute_ "echo \"$binroot/zsh\" | sudo tee -a /etc/shells >/dev/null"
        _execute_ "sudo chsh -s \"${binroot}/zsh\" ${USER} >/dev/null 2>&1"
        notice "Restart your shells to use zsh"
      else
        _execute_ "sudo chsh -s \"${binroot}/zsh\" ${USER} >/dev/null 2>&1"
        notice "Restart your shells to use zsh"
      fi
...
This script attempts to install zsh shell.
